...
class Clasp {
  ...
  async callGASAPI(scriptId, method, endpointSuffix = '', body) {
    ...
    if (this.credentials.expiryDate && Date.now() > this.credentials.expiryDate) {
      console.warn('GCA4G: Access token expired, attempting automatic refresh...');
      try {
        // Service Workerにトークン更新をリクエスト
        const response = await chrome.runtime.sendMessage({ type: 'REFRESH_TOKEN' });
        if (response.success) {
          // 更新された認証情報を再読み込み
          await this.loadCredentials();
          console.log('GCA4G: Token automatically refreshed');
        } else {
          throw new Error('Automatic token refresh failed: ' + response.error);
        }
      } catch (refreshError) {
        console.error('GCA4G: Failed to refresh token:', refreshError);
        throw new Error('Access token has expired and automatic refresh failed. Please re-authenticate.');
      }
    }
